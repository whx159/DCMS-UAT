...
<table align="left"  width="100%">
		...
		<tr>
			<td colspan="2">
				<@CommonQueryMacro.DataTable id ="datatable"   fieldStr="txnid[150],workdate,txntime,txndate,fpsTxnTime,merid[150],mername,paytype,fpstxid[200],pymengid[200],currency[150],amount[150],bamount[150],status[150],amtrefunded,amtpendingrefund,amtrejectedrefund,posseroalno,posstaffid,proxyid"  width="100%"   hasFrame="true"/>
			</td>
		</tr>
	</table>
...
<script language="javascript">
	...
	function doDetail(fpstxid,flag){
		...
		var type = HKGenQRtxnList_interface_dataset.getValue("trantype");	
	    if (record) {
	        HKGenQRtxnList_dataset.setRecord(record);
	    }
	    var txnid = record.getValue("txnid");
		var merid = record.getValue("merid");
		showWin("Refund Detail","/fpages/hsbc/ftl/RefundDetail.ftl?fpstxid=" + fpstxid + "&txnid=" +txnid+"&merid="+merid+"&flag="+flag+"&type="+type,"window","flushPage()",window);
	}

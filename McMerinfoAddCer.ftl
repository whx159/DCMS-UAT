                    ...
                    <table frame=void class="grouptable" width="100%">
                        <tr>
                           <td align="right" nowrap class="labeltd" width="15%">API Client ID</td>
                            <td class="datatd" width="35%"><@CommonQueryMacro.SingleField fId="clientidsel"/></td>
...
<script language="javascript">
	...
	function bindCertificate(id){
		...
		if(mertype == "2"){
			McPubkeyList_dataset.setFieldReadOnly("select",true);
		} else {
			McPubkeyList_dataset.setFieldReadOnly("select",false);
			McPubkeyList_dataset.setValue("select",false);
		}
	...
	function btConfirm1_onClickCheck(button){
		var selectCerno = McMerinfoList_dataset.getValue("selectCerno");
	   	if (!selectCerno) {
	   		alert("请选择需要需要绑定的证书！");
	   		return false;
		}else{
			...
			if("yes"==flag){
				McMerinfoList_dataset.setParameter("flag","0");
			}else{
				McMerinfoList_dataset.setParameter("flag","1");
			}
			return true;
		}
	}	

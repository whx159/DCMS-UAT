...
<script language="javascript">
    ...
    function filetype_DropDown_onSelect(dropdown,record,editor){
		...
		}else if(fileType == "02" || fileType == "08" || fileType == "11"){
    		ReportFile_dataset.setFieldReadOnly("merid",false);
    		ReportFile_dataset.setFieldRequired("merid",true);
    		ReportFile_dataset.setValue("type","");
    		ReportFile_dataset.setFieldReadOnly("type",false);
	...
	function btAdd_onClick() {
		...
		if(brno == "HKHBAP"){
			...
			if(fileType == "01" || fileType == "02" || fileType == "04" || fileType == "03"){
				if(merid == ""){
					alert("Merchant ID is empty!");
					return;
				}
		...
		if(fileType != "06" && createDate != ""){
			var date = new Date();
			var currentdate = formatDate(date);
			if(currentdate < createDate){
				alert("Cannot generate report in the future time");
				return false;
			}
		}
		if(brno == "HKHBAP" && createDate != "" && endDate != ""){
			if(createDate>endDate){

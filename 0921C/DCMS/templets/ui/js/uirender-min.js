...
function exporter(c, b) {
    ...
    this.openExpWinDownLoad = function(e) {
    	this.expType = e
//    	this.expWin = $(this.expwinid).hide();
    	var d = this;
    	d.download();
//    	this.expWin.dialog(
//    		{	//display:none,
...
function exportQuery(dataset){
	var m = document.getElementById("query_download");
    if (m == null || m == "undefined") {
        m = document.createElement("DIV");
        m.id = "query_download";
        m.style.visibility = "hidden"
	...
	e.name = "exportQuery";
    e.method = "post";
    e.action = "/DCMS/batchImport/download.spring";
    e.style.visibility = "hidden";
    var t = getDatasetByID(dataset+"_interface_dataset");
    if (t) {
        for (var l = 0; l < t.fields.fieldCount; l++) {
            e.insertAdjacentHTML("beforeEnd", '<input type="hidden" name="'
                + t.getField(l).fieldName + '" value="'+ t.getString(l) + '"/>');
        }
        e.insertAdjacentHTML("beforeEnd", '<input type="hidden" name="fileflag" value="'+ dataset + '"/>');
    }
    m.appendChild(e);
    e.submit();
}
...
function checkAllpage(d, c, b) {
    if (c.length){
        ...
        for (i=0;i<c.length;i++){
            c[i].disabled = d.checked;
            c[i].readOnly = d.checked;
        }
    }
    else{
        c.disabled = d.checked;
        c.readOnly = d.checked;
    }

    if (b.length){
        var i = 0;
        for (i=0;i<b.length;i++){
            b[i].disabled = d.checked;
            b[i].readOnly = d.checked;
        }
    }
    else{
        b.disabled = d.checked;
        b.readOnly = d.checked;
    }
}
...
function initCheckboxs(element) {
    ...
    while (_record) {
    	if(zqq){
    		checkboxGroup.push('<span style="width:15px;height:10px;display:inline-block;" /><input id="' + fieldName
    	            + '" type="checkbox" name="' + fieldName + rid + '" value="'
    	            + _record[0] + '" >' + _record[1]+ '</input><br />');
    	        _record = _record.getNextRecord();
    	}else{
	        checkboxGroup.push('<span style="width:1px;height:10px;display:inline-block;" /><input id="' + fieldName
	            + '" type="checkbox" name="' + fieldName + rid + '" value="'
	            + _record[0] + '" >' + _record[1] + "</input>" + "<br />");
	        _record = _record.getNextRecord();
    	}
...
function initCheckboxsBak(element) {
    var fieldName = element.getAttribute("name");
    eval("var checkboxDataset=" + element.getAttribute("dataField")
        + "_CheckboxDataset");
    var resultDataset = element.getAttribute("componentDataset");
    ...
    f
        .combo({
            ...
            onInputText: function (k) {
                if (f.attr("readonly") == "readonly" || f.attr("readonly") == true || f.attr("readonly") == "true"){
                    return;
                }
                var j = f.combo("getValue");
                var ii = validDateOrTime(k, c == "datebox" ? "date" : "timestamp");
                /**
                 * 输入的格式不合法
                 */
                if (ii == false){
    ...
    if (o.attr("extra") != "dropDownSelect") {
        ...
        if (p != c.getString(r)) {
            ...
            if (o.attr("editType") == "numberbox") {
                ...
                if (g.length > 17 || g.indexOf("e") > -1 || g.indexOf("E") > -1) {
        ...
        if (o.attr("editType") == "file"){
            var _fileName = o.val();
            var _extends = o.attr("extends");
            if (!_fileName){
                e = true;
                alert(constFileUpload.replace("%a", o.attr("dataField")));
            }
            else if(_extends){
                var _extends_arr = _extends.split(";");
                var _index = _fileName.lastIndexOf(".");
                var _ext = _fileName.substr(_index + 1);
                var _flag = false;
                for (var i=0;i<_extends_arr.length;i++){
                    if(_ext == _extends_arr[i]){
                        _flag = true;
                        break;
                    }
                }
                if (_flag == false){
                    alert(constFileUploadExtends.replace("%a", _ext).replace("%b", _extends));
                }
            }
...
function initSelectAndDicClick(d, g, f) {
    ...
    var zqqf = d.getAttribute("dataField");
    var zqqg = h.getField(zqqf);
    var idaddzqq = $(d);
    idaddzqq.attr("idaddzqq",zqqg.fieldName);
...
function initSelectCQCQ(u, c, P, N) {
    ...
    Q.push("<div id='div_" + A
        + "' class='datagrid-view2' style='width:100%;left:0px;'>");
    Q.push("<div id='head_" + A
        + "' class='datagrid-header' style='height:24px'>");
    Q.push("<table class='datagrid-htable'><tbody class='datagrid-header'><tr class='datagrid-header-row'>");
    var C = 0;
    var M = 100;
    ...
    Q.push("<div id='body_" + A + "' class='datagrid-body' style='width:100%;'>");
    Q.push("<table class='datagrid-btable' cellspacing='0' cellpadding='0' border='0' style='width:100%;;table-layout:auto' ><tbody>");
    var J;
    var g;
    var L;
    var T = w.pageSize || 10;
    ...
    Q.push('<td><a href="javascript:void(0);" class="l-btn l-btn-plain l-btn-disabled" id="first_'
        + A
        + '" ><span class="l-btn-left"><span class="l-btn-text"><span class="l-btn-empty pagination-first">&nbsp;</span></span></span></a></td>');
    Q.push('<td><a href="javascript:void(0);" class="l-btn l-btn-plain l-btn-disabled" id="pre_'
        + A
    ...
    Q.push('<td><input id="current_' + A + '" class="pagination-num" type="text" value="1" size="2"></td>');
    Q.push('<td>/<span class="pagination-sumNum" id="sumNum_' + A + '" style="padding-right:6px;">2</span></td>');
    Q.push('<td><div class="pagination-btn-separator"></div></td>');
    Q.push('<td><a href="javascript:void(0)" class="l-btn l-btn-plain" id="next_'
        + A
        + '"><span class="l-btn-left"><span class="l-btn-text"><span class="l-btn-empty pagination-next">&nbsp;</span></span></span></a></td>');
    Q.push('<td><a href="javascript:void(0)" class="l-btn l-btn-plain" id="last_'
        + A
        + '"><span class="l-btn-left"><span class="l-btn-text"><span class="l-btn-empty pagination-last">&nbsp;</span></span></span></a></td>');
    Q.push('<td><div class="pagination-btn-separator"></div></td><td><a href="javascript:void(0)" class="l-btn l-btn-plain" id=""><span class="l-btn-left"><span class="l-btn-text"><span class="l-btn-empty icon-clear" title="clear">&nbsp;</span></span></span></a></td>');
    Q.push('</tr></tbody></table><div class="pagination-info"></div><div style="clear:both;"></div></div>');
    Q.push("</div>");
    var f = $(Q.join(""));
    ...
    $("#sumNum_" + A, f).html(w.autoPageFlag ? w.autoPageCount : w.pageCount);
    $("#current_" + A, f).val(w.autoPageFlag ? w.autoPageIndex : w.pageIndex);
    var e = w.pKey || [];
    if (e.length > 0) {
        ...
        for (var R = 0; R < e.length; R++) {
            if (e[R].name && e[R].name.startWith("value")) {
                ...
                var x = $(
                    ...
                    .validatebox();
                var U = $("<tr style='width: 100%;'></tr>").append(
                    $("<td style='width:20%;' align='right'></td>").append(p)).append(
                    $("<td style='width:80%;' align='left'></td>").append(x));
                U.appendTo(z)
            }
        }
            ...
            enter: function () {
                {
                    ...
                    if (F) {
                        ...
                        q(u, y.fieldMap, Y, f.find("tr.datagrid-row-selected").data("record"))
                    }
                    Y.CachedRecord = f.find("tr.datagrid-row-selected").data("record");
                    if (!F) {
                        I.combo("hidePanel")
                    }
                }
...
function cqResize(h) {
    ...
    h.find(".datagrid-body").height(360 - f - g - e - 5);
...
function initSelectAndDic(element) {
    ...
    select.combobox({
        ...
        onInputText: function (value) {
            var opts = select.combobox("options");
            if (value != "" && opts.maindataset.getValue(select.attr("dataField") + "Name") == value){
                return;
            }
...
//			$(this).find(".combobox-item:first[value=]").css("height", "14px");
    ...
    var fieldName = select.attr("dataField");
    eval(fieldName + "_Json=json;")
